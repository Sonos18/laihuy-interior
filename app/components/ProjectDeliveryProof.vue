  ...
  >
    <div class="shell">
      <div class="grid gap-10 md:grid-cols-[0.72fr_1.28fr] md:items-end">
        ...
        <p
          v-reveal="120"
          class="measure-lead reveal text-lg leading-8 text-[var(--fg-light-muted)]"
        >
          {{ t({
      ...
      <ul class="mt-10 flex flex-wrap border-y border-[var(--rule-light)]">
        <li
          v-for="scope in scopeItems"
          :key="scope.key"
          :data-scope-key="scope.key"
          class="flex items-center gap-3 border-r border-[var(--rule-light)] px-5 py-4 text-sm font-black text-[var(--fg-light)]"
        >
          <Icon
            :name="scope.icon"
            class="h-5 w-5 text-[var(--accent-light)]"
          />
          {{ scope.label }}
        </li>
      </ul>
      ...
      >
        <li
          ...
          :key="phase.key"
          class="relative border-t border-[var(--rule-light)] py-5 md:border-l md:border-t-0 md:px-5"
        >
          <span class="text-xs font-black text-[var(--accent-light)]">{{ String(index + 1).padStart(2, '0') }}</span>
          <Icon
            :name="phase.icon"
            class="mt-3 h-6 w-6 text-[var(--fg-light)]"
          />
          <p class="mt-3 text-sm font-black text-[var(--fg-light)]">
            {{ phase.label }}
          </p>
        </li>
      </ol>
      ...
      <div
        ...
        class="mt-14 border-t border-[var(--rule-light)] pt-8"
      >
        <p class="text-xs font-black uppercase tracking-[0.16em] text-[var(--accent-light)]">
        ...
        <div class="mt-6 grid gap-8 md:grid-cols-3">
          <article
            ...
            class="border-t-2 border-[var(--bronze)] pt-5"
          >
            <Icon
              :name="proof.icon"
              class="h-6 w-6 text-[var(--accent-light)]"
            />
            <h3 class="mt-4 text-lg font-black text-[var(--fg-light)]">
              {{ proof.title }}
            </h3>
            <p class="mt-3 text-sm leading-6 text-[var(--fg-light-muted)]">
              {{ proof.description }}
            </p>
          </article>
        </div>
